/**
 * Import campaign replied creators into creator_master.
 *
 * Idempotent: skips creators already processed (source='campaign_reply' in pricing history).
 * Only adds creators who have a clear price in their reply.
 *
 * Flow:
 *   1. Query campaign replied emails from prod DB (brand schema)
 *   2. Filter out already-processed creators (pricing history exists)
 *   3. Use OpenAI to extract price_per_video + price_comment
 *   4. New creators with price: insert creator_master + pricing_history + TikTok enrich
 *   5. Existing creators with price: update pricing_history only
 *   6. No price in reply: skip entirely (no DB writes)
 *
 * Usage:
 *   npx tsx scripts/import-campaign-replies.ts --campaign-id=818             # live run
 *   npx tsx scripts/import-campaign-replies.ts --campaign-id=818 --dry-run   # preview
 *   npx tsx scripts/import-campaign-replies.ts --campaign-id=818 --skip-enrich
 */
...
if (!campaignIdArg) {
  console.error("Usage: npx tsx scripts/import-campaign-replies.ts --campaign-id=<ID>");
  process.exit(1);
}
const CAMPAIGN_ID = parseInt(campaignIdArg.split("=")[1]);
const BATCH = 5;
...
let tikTokCreds: { appKey: string; appSecret: string; accessToken: string; shopId: string; shopCipher: string } | null = null;

async function getTikTokCreds() {
  if (tikTokCreds) return tikTokCreds;
  const tikDb = createClient(process.env.TIKTOK_DB_SUPABASE_URL!, process.env.TIKTOK_DB_SUPABASE_KEY!);
  const { data: rows } = await tikDb.from("user_tiktok_info").select("access_token, shop_cipher").eq("shop_id", SHOP_ID).is("deleted_at", null).order("updated_at", { ascending: false }).limit(1);
  if (!rows?.length) throw new Error("No TikTok credentials found");
  tikTokCreds = { appKey: APP_KEY, appSecret: APP_SECRET, accessToken: rows[0].access_token, shopId: SHOP_ID, shopCipher: rows[0].shop_cipher };
  return tikTokCreds;
}

async function enrichFromTikTok(handle: string): Promise<{ gmv: number | null; avg_view: number | null; followers: number | null } | null> {
  try {
    ...
    const resp = await fetch(`${rawUrl}&sign=${sign}&timestamp=${ts}`, {
      method: "POST",
    ...
    const match = creators.find((c: { username: string }) => c.username.toLowerCase() === cleanHandle.toLowerCase()) || creators[0];
    let gmvVal: number | null = null;
    if (match.gmv?.amount) { gmvVal = parseFloat(match.gmv.amount); if (isNaN(gmvVal)) gmvVal = null; }
    return { gmv: gmvVal, avg_view: match.avg_ec_video_view_count ?? null, followers: match.follower_count ?? null };
  } catch (err) {
...
async function extractPriceAI(handle: string, replyText: string): Promise<{ price_per_video: number | null; price_comment: string | null }> {
...
async function main() {
  ...
  console.log(`Campaign: ${CAMPAIGN_ID} | Mode: ${isDryRun ? "DRY RUN" : "LIVE"}${skipEnrich ? " | Skip enrich" : ""}\n`);
  ...
  console.log(`  Total replied: ${replies.length} (with handle: ${withHandle.length}, no handle: ${replies.length - withHandle.length})\n`);

  // Step 2: Filter out already-processed creators
  console.log("Step 2: Filtering already-processed creators...");
  const handles = withHandle.map((r) => r.handle!.toLowerCase().replace(/^@/, "").replace(/^\.+/, ""));

  // Get existing creator_master records
  ...
  (existingCreators || []).forEach((c) => existingMap.set(c.handle.toLowerCase(), { id: c.id, price_per_video: c.price_per_video, email: c.email }));

  // Get already-processed handles (have campaign_reply pricing history for THIS campaign)
  const existingIds = (existingCreators || []).map((c) => c.id);
  const alreadyProcessedSet = new Set<string>();

  if (existingIds.length > 0) {
    const { data: processedHistory } = await trackerDb
      .from("creator_pricing_history")
      .select("creator_master_id")
      .eq("source", "campaign_reply")
      .eq("campaign_id", CAMPAIGN_ID)
      .in("creator_master_id", existingIds);

    const processedIds = new Set((processedHistory || []).map((h) => h.creator_master_id));
    (existingCreators || []).forEach((c) => {
      if (processedIds.has(c.id)) alreadyProcessedSet.add(c.handle.toLowerCase());
    });
  }

  // Filter: exclude already-processed
  const toProcess = withHandle.filter((r) => {
    const clean = r.handle!.toLowerCase().replace(/^@/, "").replace(/^\.+/, "");
    return !alreadyProcessedSet.has(clean);
  });

  console.log(`  Already processed (skipped): ${alreadyProcessedSet.size}`);
  ...
  // Step 3: Extract pricing + insert/update (only if price found)
  console.log("Step 3: Extracting prices and updating DB...\n");
  const stats = { new_created: 0, existing_updated: 0, no_price: 0, enriched: 0, enrich_failed: 0, error: 0 };
  const enrichHandles: string[] = [];

  for (let i = 0; i < toProcess.length; i += BATCH) {
    const batch = toProcess.slice(i, i + BATCH);
    const label = `[${i + 1}-${Math.min(i + BATCH, toProcess.length)}/${toProcess.length}]`;
    process.stdout.write(`${label} `);

    await Promise.all(
      batch.map(async (r) => {
        const cleanHandle = r.handle!.toLowerCase().replace(/^@/, "").replace(/^\.+/, "");
        const isExisting = existingMap.has(cleanHandle);

        const { price_per_video, price_comment } = await extractPriceAI(cleanHandle, r.reply_text);

        // No price found: skip entirely
        if (!price_per_video) {
          stats.no_price++;
          if (isDryRun) process.stdout.write(`\n  [SKIP] @${cleanHandle} → no price`);
        ...
        if (isDryRun) {
          ...
          process.stdout.write(`\n  [${tag}] @${cleanHandle} → $${price_per_video}${price_comment ? ` (${price_comment})` : ""}`);
        ...
        try {
          if (isExisting) {
            ...
            const updates: Record<string, unknown> = { updated_at: new Date().toISOString(), price_per_video };
            if (!existing.email && r.creator_email) updates.email = r.creator_email;
            if (price_comment) updates.price_comment = price_comment;

            await trackerDb.from("creator_master").update(updates).eq("id", existing.id);
            await trackerDb.from("creator_pricing_history").upsert({
              creator_master_id: existing.id,
              recorded_at: new Date().toISOString().split("T")[0],
              price_per_video, price_comment, source: "campaign_reply", campaign_id: CAMPAIGN_ID,
            }, { onConflict: "creator_master_id,recorded_at" });

            stats.existing_updated++;
            process.stdout.write(`U`);
          } else {
            // New creator: only create if price exists (guaranteed here)
            const { data: created, error: createErr } = await trackerDb
              .from("creator_master")
              .upsert({
                handle: cleanHandle,
                email: r.creator_email || r.reply_email,
                price_per_video,
            ...
            if (createErr || !created) {
              console.error(`\n  ERROR creating @${cleanHandle}:`, createErr?.message);
              stats.error++;
              return;
            }
  ...
  if (!skipEnrich && !isDryRun && enrichHandles.length > 0) {
    console.log(`Step 4: Enriching ${enrichHandles.length} new creators with TikTok API...\n`);
    for (let i = 0; i < enrichHandles.length; i++) {
      const handle = enrichHandles[i];
      process.stdout.write(`  [${i + 1}/${enrichHandles.length}] @${handle} ... `);

      const data = await enrichFromTikTok(handle);
      if (!data) { process.stdout.write(`not found\n`); stats.enrich_failed++; continue; }

      const { data: creator } = await trackerDb.from("creator_master").select("id, price_per_video").eq("handle", handle).single();
      if (!creator) { stats.enrich_failed++; process.stdout.write(`master not found\n`); continue; }
      ...
      if (creator.price_per_video && data.avg_view) {
        const tier = calculateTier(Number(creator.price_per_video), Number(data.avg_view), Number(data.gmv) || 0);
      ...
      await trackerDb.from("creator_pricing_history").upsert({
        ...
        recorded_at: new Date().toISOString().split("T")[0],
        gmv: data.gmv, avg_view: data.avg_view, followers: data.followers,
      ...
      stats.enriched++;
      process.stdout.write(`GMV=${data.gmv ?? "N/A"} avg_view=${data.avg_view ?? "N/A"} followers=${data.followers ?? "N/A"}\n`);
      await new Promise((r) => setTimeout(r, 300));
    }
  } else if (skipEnrich) {
    console.log("Step 4: Skipped TikTok enrichment (--skip-enrich)\n");
  } else if (isDryRun) {
    console.log(`Step 4: Would enrich ${enrichHandles.length} new creators with TikTok API\n`);
  }
  ...
  console.log(`  Existing price updated : ${stats.existing_updated}`);
  console.log(`  No price (skipped)     : ${stats.no_price}`);
  console.log(`  Already processed      : ${alreadyProcessedSet.size}`);
